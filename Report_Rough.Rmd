...
<br>
<br>
#### Computed Weighted Summary Tables
...
counter <- function(x){
  ...
  out <- matrix(data = NA, 
                ...
                ncol = 3)
  colnames(out) <- c("Personal Doctor Count", "Insurance Count", "Total Respondents")
...
x <- t11.counts["ME",]
colnames(x) <- c("Personal Doctor Count", "Insurance Count", "Total Respondents")
...
t11.std[,1] <- round(1000*t11.std[,1]/t11.std[,3], digits = 0)
t11.std[,2] <- round(1000*t11.std[,2]/t11.std[,3], digits = 0)

t12.std <- data.frame(t12.counts)
t12.std[,1] <- round(1000*t12.std[,1]/t12.std[,3], digits = 0)
t12.std[,2] <- round(1000*t12.std[,2]/t12.std[,3], digits = 0) 

t13.std <- data.frame(t13.counts)
t13.std[,1] <- round(1000*t13.std[,1]/t13.std[,3], digits = 0)
t13.std[,2] <- round(1000*t13.std[,2]/t13.std[,3], digits = 0) 

t14.std <- data.frame(t14.counts)
t14.std[,1] <- round(1000*t14.std[,1]/t14.std[,3], digits = 0)
t14.std[,2] <- round(1000*t14.std[,2]/t14.std[,3], digits = 0) 

t15.std <- data.frame(t15.counts)
t15.std[,1] <- round(1000*t15.std[,1]/t15.std[,3], digits = 0)
t15.std[,2] <- round(1000*t15.std[,2]/t15.std[,3], digits = 0)

AY.std <- data.frame(AY.counts)
AY.std[,1] <- round(1000*AY.std[,1]/AY.std[,3], digits = 0)
AY.std[,2] <- round(1000*AY.std[,2]/AY.std[,3], digits = 0) 

```

We will standardize the values to counts per 1000 respondents so the Maine 2011 observation will become:

```{r, echo=F, results="asis"}

x[1] <- round(1000*x[1]/x[3], digits = 0)
x[2] <- round(1000*x[2]/x[3], digits = 0)
